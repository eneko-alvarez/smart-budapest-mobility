...
\author{
    ...
    \normalsize Business Intelligence Course -- Academic Year 2024-2025
}
\date{\today}
...
\begin{abstract}
This project implements a comprehensive Business Intelligence system for Budapest's public transport network, integrating real-time vehicle tracking data from BKK Futár API with weather conditions from OpenWeatherMap. The system processes over 850 vehicles every 10 minutes, accumulating 2.18 million transport events over 14 days. Using a three-layer ELT architecture with TimescaleDB, Apache Airflow orchestration, and Metabase dashboards, it enables data-driven decision-making for mobility planning and operational efficiency.
\end{abstract}
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item \textbf{Staging}: Raw API responses (JSONB) in TimescaleDB hypertables with 7-day retention
    \item \textbf{Raw}: Cleaned/validated data with 90-180 day retention policies
...
Fact & \texttt{fact\_transport\_usage} & 2.18M vehicle observations (time\_key, route\_key, vehicle\_id, delay) \\
...
All pipelines orchestrated with Apache Airflow 2.7 (scheduling, monitoring, error handling, retry logic).
...
\textbf{Key Insights (preliminary, 14-day period)}: (1) Most activity concentrated during morning (7-9 AM) and evening (4-7 PM) rush hours. (2) Trams show higher average speeds than buses, likely due to dedicated lanes. (3) Significant reduction in weekend activity, with peaks shifting to midday hours.

\subsection{Dashboard 2: Usage Patterns}
...
\subsection{Dashboard 3: Weather Impact}
...
\textbf{Libraries}: psycopg2 (DB), requests (API), pandas (data), pytest (testing)

\subsection{Deployment}
\textbf{Prerequisites}: Docker Desktop, 8GB RAM, API keys (BKK, OpenWeatherMap)

\textbf{Setup}: (1) Clone repo, create \texttt{.env} with keys. (2) \texttt{docker-compose up -d}. (3) Auto-init DB schema. (4) Load GTFS: \texttt{python python/extractors/gtfs\_loader.py}. (5) Access: Airflow (localhost:8080), Metabase (localhost:3000), PostgreSQL (localhost:5432).
...
\begin{enumerate}[leftmargin=*, itemsep=0pt]
    ...
    \item \textbf{Limited Weather History}: Weather collection started 5 days after transport. \textit{Impact}: Only 14 days overlap for correlation. \textit{Mitigation}: Ongoing collection; target 6-12 months for ML.
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item \textbf{Automated Collection}: 850+ vehicles/10min, 2.18M events over 14 days, zero manual intervention
    \item \textbf{Robust ETL}: 3 Airflow pipelines running reliably during collection period, with error handling and retry logic
    \item \textbf{Star Schema DWH}: Fast interactive queries on millions of rows using TimescaleDB optimizations
    \item \textbf{Actionable Dashboards}: 3 Metabase dashboards enabling data exploration and preliminary insights
...
\textbf{Short-Term (1-3 months)}: Stop inference via geofencing; alerting/anomaly detection; materialized views for dashboard performance.

\textbf{Medium-Term (3-6 months)}: Accumulate 6-12 months data for seasonal patterns; statistical correlation tests (Pearson, Spearman); multi-city expansion (Vienna, Prague, Berlin).

\textbf{Long-Term (6-12 months)}: ML demand forecasting (ARIMA, Prophet, LSTM); "cursed routes" congestion prediction; real-time streaming (Kafka); public API for third-party developers.

\vspace{0.5cm}

\noindent\textbf{Project Metadata}: Author: Eneko Alvarez | Institution: BME | Course: Business Intelligence | Year: 2024-2025 | Period: Nov 15-29, 2025 (14 days) | Records: 2,179,435 transport + 88,244 weather | Repository: \url{https://github.com/eneko-alvarez/smart-budapest-mobility}
